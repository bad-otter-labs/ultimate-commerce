...
final class PersonalDataRegistry
{
    ...
    public function register(
        ...
        if (!preg_match('/^[a-z][a-z0-9_-]{0,63}$/', $key)) {
            return self::error('uc_privacy_handler_key', 'Personal-data handler key is invalid.');
        }
        if ($name === '' || strlen($name) > 120) {
            return self::error('uc_privacy_handler_name', 'Personal-data handler name is invalid.');
        }
        if (!DataRetention::valid($retention)) {
            return self::error('uc_privacy_retention', 'Personal-data retention class is invalid.');
        }
        if (isset($this->handlers[$key])) {
            return self::error('uc_privacy_handler_duplicate', 'A personal-data handler is already registered with this key.');
        }
